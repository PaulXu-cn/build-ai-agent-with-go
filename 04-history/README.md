...
**但前缀太短是命中不了的。** 缓存有最小粒度，不足一个存储单元的前缀落不了盘。
上面那次跑出来的 `40 → 100` 是假服务按"逐条前缀命中"模拟的理想值；真拿几十个 token
的短对话去跑，`prompt_cache_hit_tokens` 全程是 0，要等上下文涨到几百上千才会开始省事。
官方的命中规则见参考里的 DeepSeek 链接。
