...
// 文档： https://developers.openai.com/api/docs/guides/migrate-to-responses
...
// ResponsesUsage token 用量。缓存字段是嵌套的，和 Chat Completions 一个路子。
...
func chatResponses(apiKey, baseURL, model, prompt string) (string, error) {
	...
	// output 是 typed 数组：取 type=message 项里 type=output_text 的文本
	...
	printUsage(resp.Usage.InputTokens, resp.Usage.InputTokensDetails.CachedTokens, resp.Usage.OutputTokens)
